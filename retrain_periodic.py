...
from model.model_vq_periodic import VQ_Align
...
def main(args):
    ...
    try:
        if init_from == 'scratch':
            ...
            model = VQ_Align(encoder_conf, decoder_conf, "/workspace/NeuroLM-Decode/VQ.pt")
            start_epoch = 0
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for step, (batch) in enumerate(data_loader_train):
                ...
                lr_slow = lr_schedule_values[max([0,iter_num-700])] if args.decay_lr else args.learning_rate
                ...
                ################################
                if step>250 and np.random.random()>0.9:
                    ...
                    noise_scale_percent =  min([0.5,np.random.random()**2/2])
                    X = X*(1-noise_scale_percent**2)**0.5+ noise*(abs_scale)*noise_scale_percent
                
                # Convert to numpy for frequency domain calculations
                # with torch.no_grad():
                #     #y_freq_norms = Y_freq.norm(dim=-1, keepdim=True)
                #     # Convert to numpy, perform FFT operations, then convert back to tensor
                #     y_raw_np = Y_raw.cpu().detach().float().numpy()
                    
                #     # Calculate power spectrum using numpy's rfft
                #     y_freq_complex_np = np.fft.rfft(y_raw_np, axis=-1)[:,:,:-1]
                #     y_freq_power_np = (np.abs(y_freq_complex_np)**2)
                #     with torch.no_grad():
                #         # Convert back to tensor and move to the same device as y_freq
                #         y_freq = torch.tensor(y_freq_power_np, device=Y_freq.device, dtype=Y_freq.dtype)
                        
                #         # Create a weight tensor that decreases from 1 to 0 based on the last dimension index
                #         freq_weights = 1.0 - torch.arange(Y_freq.shape[-1], device=Y_freq.device) / float(Y_freq.shape[-1] - 1)
                #         freq_weights = freq_weights.view(1, 1, -1).expand_as(y_freq)
                #         y_freq = y_freq * freq_weights
                
                #         Y_freq = y_freq/y_freq.norm(dim=-1,keepdim=True)#*y_freq_norms
